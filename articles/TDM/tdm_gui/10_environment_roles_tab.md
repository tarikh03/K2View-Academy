...
1. Click **Testers** and select a user from the list or type manually the user name.

2. Click **Testers** again and select another user, or type manually a user name,  if needed.

**Notes**

- A tester can be attached to only one role per environment and cannot be attached to different environment's roles in the same environment.
